...
const REQUIREMENT_ACTION_HELP = `Usage: code-factory-cli requirement action [options]

Apply one lifecycle action to a requirement proposed by this RD Agent. The
target Requirement must be in a state that permits the selected action.

Required options:
  --requirement-id ID      Proposed child Requirement ID
  exactly one of:
    --start                Start or retry its RD Agent
    --stop                 Interrupt its running RD Agent (like Steering)
    --delete               Cancel it before execution starts
    --done                 Confirm it after successful execution
...
function parseProposedRequirementAction(
  args: readonly string[],
  runtime: CodeFactoryCliRuntime,
): { targetRequirementId: string; body: Record<string, unknown> } {
  const values = parseOptions(args, REQUIREMENT_ACTION_HELP, {
  ...
  const actions = (['start', 'stop', 'delete', 'done'] as const)
    .filter((action) => values[action] === true);
  if (actions.length !== 1) {
    throw new CliUsageError(
      'Provide exactly one of --start, --stop, --delete, or --done',
      REQUIREMENT_ACTION_HELP,
    );
  }
  return {
...
export async function runCodeFactoryCli(
  ...
  try {
    ...
    } else if (command === 'requirement action') {
      help = REQUIREMENT_ACTION_HELP;
      if (writesHelp(args.slice(2))) {
        runtime.writeOut(`${help}\n`);
        return 0;
      }
      apiBaseUrl(runtime.environment);
      const action = parseProposedRequirementAction(args.slice(2), runtime);
      endpoint = `/agent/requirements/${encodeURIComponent(action.targetRequirementId)}/action`;
      body = action.body;
