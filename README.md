...
4. Agent Triggers listen or poll for external events, normalize and deduplicate them, and route them into the Requirement conversation. The current built-in triggers poll registered PRs whose last stored state is Draft or Open, observing GitHub state, comments and reviews, CI failures, and merge conflicts. A poll can capture the final transition to Closed or Merged; after that transition is stored, the PR is excluded from later polls. An idle RD session resumes immediately; a running session consumes the new messages after its current run.
