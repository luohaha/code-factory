...
The header bell enables optional desktop notifications for completed RD Runs. In a Windows browser reached through port forwarding, open the dashboard at `http://localhost:<forwarded-port>`, click the bell, and allow notifications for that site. Successful, failed, timed-out, and cancelled RD Runs show their Requirement title and outcome; clicking a notification opens the Requirement. The browser stores the opt-in locally. Keep the dashboard tab open to receive notifications; browsers require a secure context such as localhost or HTTPS, and the site's notification permission must remain granted.
