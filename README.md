...
3. The RD agent edits and tests the repository, then uses the bundled `code-factory-cli` to register any pull request it creates, propose separate follow-up work, or schedule a wake-up while a long external build or command continues.
4. Agent Triggers route normalized, deduplicated messages into the Requirement conversation. The scheduled trigger persists configurable one-time or recurring timers and sends `continue.` when due. The GitHub triggers poll registered PRs whose last stored state is Open, observing state, comments and reviews, CI failures, and merge conflicts. A poll can capture a transition to Draft, Closed, or Merged; after that transition is stored, the PR is excluded from later polls. Draft PRs are not polled, so a PR must initially be registered as Open to enter the listener. An idle RD session resumes immediately; a running session consumes the new messages after its current run.
...
Different Requirements can run concurrently, while each Requirement has at most one active RD run. Requirement state, conversations, runs, sessions, PR metadata, scheduled wake-ups, and Agent Trigger receipts are persisted in SQLite.
