...
Before opening its application database or HTTP listener, Agent Manager takes an exclusive process-lifetime lock under the canonical workspace's default data directory. Foreground and daemon-managed processes use the same lock, so a second Manager cannot bypass workspace ownership by selecting a different port, configuration file, or database. A live daemon supervisor also reserves the workspace between Manager restart attempts. The operating system releases the underlying SQLite lock if the Manager process crashes. Graceful Manager shutdown aborts all active Agent Runs, waits for each complete process tree to exit and its terminal outcome to be persisted, and only then closes SQLite and releases the workspace lock.
...
- while a Requirement remains TODO, a human may change or clear its model and reasoning effort in the dashboard; the provider remains immutable and the configuration is locked when execution starts;
- the source RD Session may apply the normal lifecycle actions to that proposed child: start or retry active work, stop a running RD Run with Steering semantics, delete TODO work, or confirm successfully executed work as DONE;
- an agent proposal is created as TODO by default, preventing uncontrolled recursive work; an explicit CLI `--start` opt-in starts its RD Session immediately, while `requirement action --start` can start it later.
