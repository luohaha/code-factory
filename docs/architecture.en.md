...
Agent Manager adds only Code Factory behavioral instructions that identify the relevant `code-factory-cli` commands. It places a private CLI launcher on the RD process's `PATH` and injects `CODE_FACTORY_API_URL`, `CODE_FACTORY_REQUIREMENT_ID`, and `CODE_FACTORY_SESSION_ID`; HTTP paths and payload schemas remain in CLI help instead of the model prompt. The CLI can read another Requirement's complete conversation or select its head, tail, or a bounded page without loading the remaining messages; it requires the target ID and rejects the current Requirement ID because that conversation is already present in the RD Session. It does not copy or replace provider-managed instructions, Skills, plugins, or local memories.
