...
`timer` is a built-in persistent Agent Trigger. Humans create `AgentTimer` resources from the Requirement chat composer or HTTP API. For every long-running process or task it starts, an RD Agent must either wait for completion during the current Run or use `code-factory-cli timer register --description DESCRIPTION` before ending the Run. Every due occurrence delivers exactly one System message with the timer ID, schedule, and description through the normal Requirement message stream. Recurring messages also include the command needed to cancel the timer. The existing delivery rules then resume an idle Session or queue the message behind its active Run.
