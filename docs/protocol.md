...
A classified Provider quota failure stores its explicit reset time without advancing the cursor. Timer, Agent Trigger, related-Agent, and Agent-proposed automatic starts remain queued for that Provider until the reset; one recovery pass then resumes each eligible Requirement once with all pending messages. Human start and reply actions are explicit immediate retries and can clear the pause early when successful.
