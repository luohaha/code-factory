...
- RD Runs time out after 60 minutes by default and Reviewer Runs after at most 30 minutes; a timeout sends `SIGTERM`, followed by `SIGKILL` after two seconds if necessary;
- one RD AgentSession may have only one active Run, while Sessions for different Requirements may run concurrently;
- Human or Reviewer messages received during an RD Run are appended to the Requirement conversation, then delivered by resuming the same native Session after the current Run;
