...
5. Watch the GitHub `Release` workflow. A matching stable tag verifies its version, publishes `@code-factory/agent-manager` to npm with provenance, and creates GitHub release notes only after npm publishing succeeds.
6. Verify the npm package and GitHub release, then smoke-test in a disposable repository:

   ```bash
   npx --yes --package @code-factory/agent-manager@0.1.0 code-factory-agent-manager --version
   npx --yes --package @code-factory/agent-manager@0.1.0 code-factory-agent-manager start --port 4310
   ```
